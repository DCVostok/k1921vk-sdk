...
static inline void _offset_mepc_to_next_instruction() {
    uint32_t mepc = csr_read_mepc();
    if (*((uint8_t*)(mepc)) & 0x3 == 0x3) {
        csr_write_mepc(mepc + 4);
    } else {
        csr_write_mepc(mepc + 2);
    }
}
...
void trap_handler(void) {
    ...
    if ((this_cause & MCAUSE_INTERRUPT_BIT_MASK) == 0) {
        ...
        if (riscv_exception_handler_map[this_cause] != 0) {
            ...
            _offset_mepc_to_next_instruction();
        } else {
